/* generated QA scenario: animation with rAF polyfill */
(async () => {
  ...
  try {
    ...
    /* headless firefox --screenshot never fires requestAnimationFrame (verified in qa_anim); polyfill it so the animated paths can run */
    window.requestAnimationFrame = cb => setTimeout(() => cb(performance.now()), 16); window.cancelAnimationFrame = id => clearTimeout(id); PT.note("rAF polyfilled with setTimeout(16)");
    await newSeason(2); for (let i = 0; i < 3; i++) ClubUI.simDay(); let e0 = errMark();
    await startGame(1, 1); $("spd").value = 10; const pc = () => GameUI.state().pitches.us + GameUI.state().pitches.them;
    $("playPitch").click(); const one = await PT.until(() => pc() === 1 && !$("playPitch").disabled, 15000); chk("한 구 (animated, 10x) completes within 15s", one, "pitches=" + pc() + " busy=" + $("playPitch").disabled);
    const P0 = pc(); for (let i = 0; i < 12; i++) $("playPitch").click(); await PT.until(() => !$("playPitch").disabled, 15000); await PT.wait(200);
    chk("12 rapid 한 구 clicks -> exactly 1 pitch (busy guard)", pc() - P0 === 1, (pc() - P0) + " pitches");
    $("spd").value = -1; chk("spd input has min=0.25 but accepts -1", $("spd").value === "-1", $("spd").value); const Pn = pc(); $("playPitch").click(); await PT.wait(3000);
    const frozen = $("playPitch").disabled && pc() === Pn; chk("EXPECT 배속 -1 does not freeze the pitch (buttons back within 3s)", !frozen, "playPitch.disabled=" + $("playPitch").disabled + " pitches " + Pn + "->" + pc());
    $("stop").click(); const resc = await PT.until(() => !$("playPitch").disabled, 5000); chk("정지 rescues the frozen pitch", resc, "playPitch.disabled=" + $("playPitch").disabled);
    ...
    const pr = fastGuard(8000); await PT.wait(30); for (let i = 0; i < 20; i++) { $("stop").click(); await PT.wait(2) } await pr; let G = GameUI.state();
    chk("fast + stop spam: stopped cleanly, buttons enabled", G.over || !$("playPitch").disabled, "over=" + G.over + " inning=" + G.inning + G.half + " pitches=" + pc());
    chk("feed announces interruption", G.over || ($("feed").textContent || "").includes("중단"), ($("feed").textContent || "").slice(0, 80));
    if (!G.over) { $("playPA").click(); const pa = await PT.until(() => !$("playPA").disabled, 25000); chk("타석 completes", pa, "pitches=" + pc()) }
    if (!GameUI.state().over) { $("playInning").click(); const br = await PT.until(() => !$("sceneBreak").hidden || GameUI.state().over, 60000); if (!br) { $("stop").click(); await PT.wait(300) } chk("이닝 reaches the break screen", br, "pitches=" + pc() + " over=" + GameUI.state().over);
      if (br && !GameUI.state().over) { $("breakPrev").click(); chk("break prev wraps to 4/4", (PT.text("#breakPageNo") || "").includes("4/4"), PT.text("#breakPageNo")); $("breakNext").click(); chk("break next wraps to 1/4", (PT.text("#breakPageNo") || "").includes("1/4"), PT.text("#breakPageNo")); $("resume").click(); await PT.wait(50); chk("다음 이닝 진행 returns to the pitch scene", !$("scenePitch").hidden, $("scenePitch").hidden) } }
    ...
    // sign buttons during play
    e0 = errMark(); await startGame(1, 1); document.querySelector(".call[data-call='bunt']").click(); document.querySelector(".dcall[data-dcall='inside']").click(); chk("bench sign buttons: no error, feed line", PT.errs.length === e0 && ($("feed").textContent || "").includes("벤치 사인"), ($("feed").textContent || "").slice(0, 40));
    { const r = await finishGame(); PT.note("sign game finished hung=" + r.hung) }
    ...
    $("playFast").click(); await PT.wait(800); chk("EXPECT 결과 바로보기 can finish the seed -1 game", GameUI.state() && GameUI.state().over, "over=" + (GameUI.state() && GameUI.state().over) + " feed=" + ($("feed").textContent || "").slice(0, 60));
    newErrs(e0, "seed-1b");

  } catch (e) { PT.note("SCENARIO EXCEPTION " + e.message + " " + (e.stack || "").slice(0, 300)); C.push({ name: "scenario exception", ok: false, got: e.message }) }
  await PT.done({ persona: "QA 파괴 테스터 · animation with rAF polyfill", checks: C, fails: C.filter(c => !c.ok).length, hangs: HANGS });
})();
