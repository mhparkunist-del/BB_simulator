...
function applyNames(){const CL=window.APP.clubLineup;if(!CL)return;if(CL.names){BANK.batters.forEach(b=>{const n=CL.names["b"+b.id];if(n){b.name=n.name;b.pos=n.pos;b.num=n.num}});BANK.pitchers.forEach(p=>{const n=CL.names["p"+p.id];if(n){p.name=n.name;p.num=n.num}})}if(CL.oppNames){BANK.opp.batters.forEach(b=>{const n=CL.oppNames[b.id];if(n){b.name=n.name;b.pos=n.pos;b.num=n.num}});BANK.opp.pitchers.forEach(p=>{const n=CL.oppNames["p"+p.id];if(n){p.name=n.name;p.num=n.num}})}BANK.clubName=CL.clubName||BANK.clubName;BANK.oppClub=CL.oppClub||null}
function roster(){const R=BANK;applyNames();const CL=window.APP.clubLineup;if(CL&&CL.order&&CL.order.length==9){pick.order=CL.order.slice();if(CL.pitcher!==null&&CL.pitcher!==undefined)pick.pitcher=CL.pitcher}
...
function renderScore(){const n=G.total;const row=(nm,sc,h,e)=>"<tr><td>"+nm+"</td>"+Array.from({length:n},(_,i)=>"<td>"+(sc[i]===undefined?"":sc[i])+"</td>").join("")+"<td><b>"+sum(sc)+"</b></td><td>"+h+"</td><td>"+e+"</td></tr>";$("breakScore").innerHTML="<table class='box'><tr><th></th>"+Array.from({length:n},(_,i)=>"<th>"+(i+1)+"</th>").join("")+"<th>R</th><th>H</th><th>E</th></tr>"+row(BANK.clubName||"우리",G.score.us,G.hits.us,G.errors.us)+row(BANK.oppClub?(BANK.oppClub.short||BANK.oppClub.name):"상대",G.score.them,G.hits.them,G.errors.them)+"</table>"}
...
function nextThree(){const o=offense();const lu=G.half=="top"?G.lineup:G.oppLineup;const list=G.half=="top"?BANK.batters:BANK.opp.batters;return [0,1,2].map(k=>{const slot=(G.idx[o]+k)%9;const p=list.find(x=>x.id===lu[slot]);return p?"<span class='nb'><small>"+(slot+1)+"번</small> "+p.name+" <em>"+p.hand+(p.pos?" · "+p.pos:"")+"</em></span>":""}).join("")}
