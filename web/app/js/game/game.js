...
function applyNames(){if(!BANK._orig){BANK._orig={b:BANK.batters.map(b=>({name:b.name,pos:b.pos,num:b.num})),p:BANK.pitchers.map(p=>({name:p.name,num:p.num})),ob:BANK.opp.batters.map(b=>({name:b.name,pos:b.pos,num:b.num})),op:BANK.opp.pitchers.map(p=>({name:p.name,num:p.num})),club:BANK.clubName}}
 const O=BANK._orig;BANK.batters.forEach((b,i)=>Object.assign(b,O.b[i]));BANK.pitchers.forEach((p,i)=>Object.assign(p,O.p[i]));BANK.opp.batters.forEach((b,i)=>Object.assign(b,O.ob[i]));BANK.opp.pitchers.forEach((p,i)=>Object.assign(p,O.op[i]));BANK.clubName=O.club;BANK.oppClub=null;
 const CL=window.APP.clubLineup;if(!CL)return;if(CL.names){BANK.batters.forEach(b=>{const n=CL.names["b"+b.id];if(n){b.name=n.name;b.pos=n.pos;b.num=n.num}});BANK.pitchers.forEach(p=>{const n=CL.names["p"+p.id];if(n){p.name=n.name;p.num=n.num}})}if(CL.oppNames){BANK.opp.batters.forEach(b=>{const n=CL.oppNames[b.id];if(n){b.name=n.name;b.pos=n.pos;b.num=n.num}});BANK.opp.pitchers.forEach(p=>{const n=CL.oppNames["p"+p.id];if(n){p.name=n.name;p.num=n.num}})}BANK.clubName=CL.clubName||BANK.clubName;BANK.oppClub=CL.oppClub||null;window.SIDECOL={us:(window.APP.themeNow&&window.APP.themeNow.color)||"#1d2a44",them:(BANK.oppClub&&BANK.oppClub.color)||"#8a1f2b"}}
function roster(){const R=BANK;applyNames();$("seed").value=1+Math.floor(Math.random()*9998);if(window.ClubUI&&window.ClubUI.gameToday&&window.ClubUI.gameToday())$("innings").value=9;const CL=window.APP.clubLineup;if(CL&&CL.order&&CL.order.length==9){pick.order=CL.order.slice();if(CL.pitcher!==null&&CL.pitcher!==undefined)pick.pitcher=CL.pitcher}
 const PS=(window.APP.clubLineup&&window.APP.clubLineup.pitcherState)||{};
 $("pitchers").innerHTML=R.pitchers.map(p=>{const st=PS[p.id];const left=st?Math.round(100*(1-st.fatigue)):null;return "<div class='pc' data-p='"+p.id+"'><b>"+p.name+" · "+p.hand+" · "+p.form+"</b>"+grades(p,["stuff","control","stamina"])+(left!==null?"<div class='stam'><span class='bar"+(left<40?" f":"")+"'><span style='width:"+left+"%'></span></span> 체력 "+left+"%"+(st.injury?" · <span class='inj'>부상</span>":"")+(st.condition!==undefined?" · 컨디션 "+Math.round(st.condition):"")+"</div>":"")+"</div>"}).join("");
 $("batters").innerHTML=R.batters.map(b=>"<div class='pc' data-b='"+b.id+"'><span class='ord' id='ord"+b.id+"'></span><b>"+(b.num!==undefined?b.num+" ":"")+b.name+" · "+(b.pos||"")+" "+b.hand+"</b>"+grades(b,["contact","power","eye","speed"])+"</div>").join("");
...
$("start").onclick=async()=>{const seed=Math.max(1,Math.min(9999,Math.floor(+$("seed").value)||1));$("seed").value=seed;const nInn=Math.max(1,Math.min(9,Math.floor(+$("innings").value)||3));$("innings").value=nInn;if(pick.order.length<9||new Set(pick.order).size<9||pick.order.includes(null)||pick.pitcher===null){feed("타순 9명(중복 없이)과 선발을 먼저 고르세요",true);return}rng=mulberry(seed);const home=!!(window.APP.clubLineup&&window.APP.clubLineup.home);G={home,inning:1,half:"top",outs:0,runners:[false,false,false],total:nInn,over:false,score:home?{us:[],them:[0]}:{us:[0],them:[]},ourStarter:pick.pitcher,oppStarter:seed%5,pc:{us:{},them:{}},usedP:{us:{},them:{}},sp:{outs:0,R:0},hits:{us:0,them:0},errors:{us:0,them:0},idx:{us:0,them:0},lineup:pick.order.slice(),oppLineup:BANK.opp.batters.map(b=>b.id),ourPitcher:pick.pitcher,oppPitcher:seed%5,pitcher:home?pick.pitcher:seed%5,pitches:{us:0,them:0},mph:{us:[0,0],them:[0,0]},box:{us:{},them:{}},pline:{us:{outs:0,H:0,R:0,K:0,BB:0},them:{outs:0,H:0,R:0,K:0,BB:0}},used:{},balls:0,strikes:0,paPitches:0};setScene("pitch");
 ...
 if(window.CUT&&!window.NOCUT){setScene("play");await window.CUT.play("intro",weBat()?{pitcher:BANK.opp.pitchers[G.oppPitcher].name,club:(BANK.oppClub&&BANK.oppClub.name)||"상대",side:"them"}:{pitcher:BANK.pitchers[G.ourPitcher].name,club:BANK.clubName||"우리",side:"us"});setScene("pitch")}drawAll(null,-2)};
async function endingCut(){if(!window.CUT||window.NOCUT)return;setScene("play");await window.CUT.play("ending",{win:sum(G.score.us)>sum(G.score.them),us:sum(G.score.us),them:sum(G.score.them),fielding:weBat()?"them":"us"});setScene("break")}
...
function feed(t,big,badge){const d=document.createElement("div");d.className="l"+(big?" big":"");d.innerHTML=(G?"<span class='t'>"+G.inning+"회"+(G.half=="top"?"초":"말")+"</span>":"")+t+(badge?"<span class='bs'>사인</span>":"");$("feed").prepend(d)}
function lamps(id,n){[...$(id).children].forEach((s,j)=>s.classList.toggle("on",j<n))}
function batter(){return weBat()?BANK.batters[G.lineup[G.idx.us%9]]:BANK.opp.batters[G.oppLineup[G.idx.them%9]]}
function weBat(){return (G.half=="top")===!G.home}          // the away side bats in the top
function offense(){return weBat()?"us":"them"}
function defense(){return weBat()?"them":"us"}
function sum(a){return a.reduce((x,y)=>x+y,0)}
function render(){if(window.SIDECOL&&typeof UNI!=="undefined"){const dc=window.SIDECOL[defense()],oc=window.SIDECOL[offense()];UNI.fielder.cap=UNI.pitcher.cap=UNI.catcher.cap=dc;UNI.batter.cap=oc}if($("sideTag"))$("sideTag").textContent=weBat()?"우리 공격":"우리 수비";$("sInn").textContent=Math.min(G.inning,G.total)+"회"+(G.half=="top"?"초":"말");$("sRuns").textContent=sum(G.score.us)+":"+sum(G.score.them);lamps("lOuts",G.outs);lamps("lBalls",G.balls);lamps("lStrikes",G.strikes);["b1","b2","b3"].forEach((b,j)=>$(b).classList.toggle("on",G.runners[j]));$("sBatter").textContent=G.over?"경기 종료":batter().name+" · 투수 "+G.pitches[defense()]+"구";if($("onDeck")){const o=offense();const lu=G.half=="top"?G.lineup:G.oppLineup;const nm=k=>{const id=lu[(G.idx[o]+k)%9];const p=G.half=="top"?BANK.batters.find(x=>x.id===id):BANK.opp.batters.find(x=>x.id===id);return p?p.name:"-"};$("onDeck").textContent=G.over?"":"대기 "+nm(1)+" · "+nm(2)}["playPitch","playPA","playInning"].forEach(k=>$(k).disabled=G.over||busy);$("offCalls").style.display=G.half=="top"?"":"none";$("defCalls").style.display=G.half=="bottom"?"":"none";renderScore()}
function renderScore(){const n=G.total;const row=(nm,sc,h,e)=>"<tr><td>"+nm+"</td>"+Array.from({length:n},(_,i)=>"<td>"+(sc[i]===undefined?"":sc[i])+"</td>").join("")+"<td><b>"+sum(sc)+"</b></td><td>"+h+"</td><td>"+e+"</td></tr>";$("breakScore").innerHTML="<table class='box'><tr><th></th>"+Array.from({length:n},(_,i)=>"<th>"+(i+1)+"</th>").join("")+"<th>R</th><th>H</th><th>E</th></tr>"+(G.home?rowThem()+rowUs():rowUs()+rowThem())+"</table>";function rowUs(){return row(BANK.clubName||"우리",G.score.us,G.hits.us,G.errors.us)}function rowThem(){return row(BANK.oppClub?(BANK.oppClub.short||BANK.oppClub.name):"상대",G.score.them,G.hits.them,G.errors.them)}}
async function pickPitch(){const b=batter().id;const top=weBat();const c=top?call:dcall;const pre=top?"":"D|";const bank=await loadBank(top?"top":"bot",G.pitcher,b);const key=pre+b+"|"+G.pitcher+"|"+c+"|"+G.balls+"|"+G.strikes;let list=bank[key]||bank[pre+b+"|"+G.pitcher+"|none|"+G.balls+"|"+G.strikes];if(!list||!list.length)throw new Error("은행 없음: "+key);
 if(list.length<3){list=list.slice();[G.balls-1,G.balls+1].forEach(bb=>{const l2=bank[pre+b+"|"+G.pitcher+"|"+c+"|"+bb+"|"+G.strikes]||bank[pre+b+"|"+G.pitcher+"|none|"+bb+"|"+G.strikes];if(l2)l2.forEach(q=>list.push(q))})}   // thin bucket: borrow neighbouring counts so the same pitch does not repeat verbatim
 if(G.paPitches>=8){const pool=[];Object.keys(bank).forEach(k=>{if(k.startsWith(pre+b+"|"+G.pitcher+"|"))bank[k].forEach(q=>{if(q.result!="foul")pool.push(q)})});if(pool.length)list=pool}   // long PA: the bank bucket may hold only fouls; resolve from the whole pair
 const u=G.used[key]||0;G.used[key]=u+1;return list[(u+Math.floor(rng()*list.length))%list.length]}
...
function paCard(){const b=batter();const top=weBat();const d=defense();const n=G.pitches[d];const m=G.mph[d];const avg=m[1]?(m[0]/m[1]).toFixed(0)+" mph":"-";
...
function maybeChangePitcher(){const d=defense();const cur=G.pitcher;const P=d=="us"?BANK.pitchers[cur]:BANK.opp.pitchers[cur];if(!P)return;const lim={S:110,A:100,B:90,C:80,D:70}[P.stamina]||85;
 const runs=G.pline[d].R-(G.pline[d].Rat||0);if((G.pc[d][cur]||0)<lim&&runs<6)return;
 const used=G.usedP[d];used[cur]=true;const starter=d=="us"?G.ourStarter:G.oppStarter;used[starter]=true;const next=[0,1,2,3,4].find(i=>!used[i]);if(next===undefined)return;
 const CL=window.APP.clubLineup||{};const rel=d=="us"?(CL.reliefNames||[]):(CL.oppReliefNames||[]);const k=Object.keys(used).length-1;const list=d=="us"?BANK.pitchers:BANK.opp.pitchers;const nm=rel[k%Math.max(1,rel.length)];if(nm){list[next].name=nm.name;list[next].num=nm.num}
 if(d=="us"&&cur===G.ourStarter){G.sp.outs=G.pline.us.outs;G.sp.R=G.pline.us.R;G.sp.done=true}
 const oldName=P.name;if(d=="us")G.ourPitcher=next;else G.oppPitcher=next;G.pitcher=next;used[next]=true;G.pline[d].Rat=G.pline[d].R;G.pc[d][next]=0;
 feed("투수 교체 · "+oldName+" → "+list[next].name+" ("+(G.pc[d][cur]||0)+"구"+(runs>=6?", "+runs+"실점":"")+")",true);render()}
function endHalf(){const o=offense();feed(G.inning+"회"+(G.half=="top"?"초":"말")+" 종료 · "+G.score[o][G.score[o].length-1]+"점",true);G.outs=0;G.runners=[false,false,false];G.balls=0;G.strikes=0;G.paPitches=0;
 ...
 else{if(G.inning>=G.total){G.over=true;render();setScene("break");renderBreak(true);feed("경기 종료 · 우리 "+sum(G.score.us)+" : 상대 "+sum(G.score.them),true);return "inning"}G.inning++;G.half="top"}
 G.pitcher=weBat()?G.oppPitcher:G.ourPitcher;const o2=offense();while(G.score[o2].length<G.inning)G.score[o2].push(0);
...
function nextThree(){const o=offense();const lu=weBat()?G.lineup:G.oppLineup;const list=weBat()?BANK.batters:BANK.opp.batters;return [0,1,2].map(k=>{const slot=(G.idx[o]+k)%9;const p=list.find(x=>x.id===lu[slot]);return p?"<span class='nb'><small>"+(slot+1)+"번</small> "+p.name+" <em>"+p.hand+(p.pos?" · "+p.pos:"")+"</em></span>":""}).join("")}
function renderBreak(over){$("breakTitle").textContent=over?"경기 종료 · 우리 "+sum(G.score.us)+" : 상대 "+sum(G.score.them):(Math.min(G.inning,G.total)+"회"+(G.half=="top"?"초":"말")+" 시작 전 · 점수와 기록");renderScore();if(!over&&$("nextUp"))$("nextUp").innerHTML="<b>"+(G.half=="top"?"우리":"상대")+" 공격 · 다음 타순</b> "+nextThree();else if($("nextUp"))$("nextUp").innerHTML="";
...
async function onePitch(){let p;try{p=await pickPitch()}catch(e){feed("이 조합의 투구 데이터가 없습니다 ("+e.message+")",true);stopFlag=true;return "pitch"}const top=G.half=="top";const used=top?call!="none":dcall!="none";$("signBadge").classList.toggle("on",used);
 ...
 if(G.fast){G.fastN=(G.fastN||0)+1;if(G.fastN%8===0)await new Promise(r=>setTimeout(r,0))}else await animate(p);const d=defense();G.pitches[d]++;G.paPitches++;G.mph[d][0]+=p.mph;G.mph[d][1]++;$("speed").textContent=p.mph.toFixed(0);feed(String(p.text).replace(/^\d+구째/,G.paPitches+"구째")+(BANK.admin?" ["+p.code+"]":""),false,used);const dd=defense();G.pc[dd][G.pitcher]=(G.pc[dd][G.pitcher]||0)+1;if(dd=="us"&&G.pitcher===G.ourStarter){G.sp.pitches=G.pc.us[G.pitcher]}if(BANK.admin)adminInfo(p);
 render();
 if(paOver){const {runs,outs,ev}=res;addRuns(runs);G.outs+=outs;
  ...
  G.idx[o]++;G.balls=0;G.strikes=0;G.paPitches=0;maybeChangePitcher();
...
async function run(mode){if(busy||!G||G.over)return;busy=true;stopFlag=false;G.fast=mode=="fast";if(G.fast){feed("결과 바로보기 · 남은 경기를 바로 진행합니다 (정지로 멈추면 그 자리부터 지휘)",true);$("playFast").textContent="진행 중…"}render();
 try{while(true){const r=await onePitch();if(G.over){await endingCut();break}if(stopFlag)break;
  if(r=="inning"&&window.CUT&&!window.NOCUT&&!G.fast){setScene("play");await window.CUT.play("inning",{incoming:weBat()?"them":"us",inning:G.inning,half:G.half});setScene("break")}
...
$("spd").onchange=()=>{$("spd").value=Math.max(0.25,Math.min(10,parseFloat($("spd").value)||1))};$("playPitch").onclick=()=>run("pitch");$("playPA").onclick=()=>run("pa");$("playInning").onclick=()=>run("inning");$("playFast").onclick=()=>run("fast");$("stop").onclick=()=>{stopFlag=true};$("stop2").onclick=()=>{stopFlag=true};
...
function animate(p){return new Promise(res=>{const t0=performance.now();const spd=Math.max(0.25,Math.min(10,parseFloat($("spd").value)||1));const T=p.flight.t[p.flight.t.length-1];const PRE=1.3;const evs=playEvents(p);const tEnd=evs.reduce((m,e)=>Math.max(m,e.t1||0,e.t||0,(e.arrive&&e.arrive.length)?e.arrive[e.arrive.length-1]:0,e.out_t||0),0);const TB=Math.max(p.batted?p.batted.flight.t[p.batted.flight.t.length-1]-T:0,tEnd>0?contactTime(p)+tEnd-T:0)+0.9;$("speed").textContent="—";drawKz(p,false);
