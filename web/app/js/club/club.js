...
function kboPlayer(kp,club,id){const r=srng(hash32(club.code+"|"+kp.name+"|"+kp.num));const kind=kp.group==="P"?"P":"B";const keys=kind==="B"?DATA.bat_keys:DATA.pit_keys;
 const peak=Math.max(0.25,Math.min(0.92,0.55+(kp.active?0.05:-0.14)+0.12*sgauss(r)));const attrs={},pot={},grades={};const head=Math.max(0,(30-kp.age)/11);
 ...
 return {id,name:kp.name,num:kp.num,age:kp.age,hand:kind==="B"?(kp.bats==="S"?"L":kp.bats):kp.throws,bats:kp.bats,throws:kp.throws,type:kind,pos:kind==="B"?kp.pos:null,pos0:kind==="B"?kp.pos:null,role:kind==="P"?(kp.active?"SP":"RP"):null,
...
/* an opponent's quick-sim strength comes from its actual first team (best 9 bats, 5 starters + bullpen), on our scale */
function oppStr(c){const rs=c.players.filter(kp=>kp.active).map((kp,i)=>kboPlayer(kp,c,90000+i));const bb=rs.filter(p=>p.type=="B").sort((a,b)=>ovr(b)-ovr(a)),pp=rs.filter(p=>p.type=="P").sort((a,b)=>ovr(b)-ovr(a));
 const m=(arr,f)=>arr.length?arr.reduce((t,p)=>t+f(p),0)/arr.length:0.5;const pf=p=>0.5*p.attrs.stuff+0.3*p.attrs.control+0.2*p.attrs.movement;
 return {bat:1.04*m(bb.slice(0,9),p=>0.4*p.attrs.contact+0.3*p.attrs.power+0.3*p.attrs.eye),pitch:1.04*(0.65*m(pp.slice(0,5),pf)+0.35*m(pp.slice(5,12),pf)),def:m(bb.slice(0,9),p=>0.7*p.attrs.defense+0.3*p.attrs.arm)}}
function buildKboClub(code){const K=window.APP.kbo;if(!K)return null;const club=K.clubs.find(c=>c.code===code);if(!club)return null;
 ...
 const opps=K.clubs.filter(c=>c.code!==code).map((c,j)=>{const r=srng(hash32("club|"+c.code+"|2026"));return {id:j+1,code:c.code,name:c.name,short:c.short,color:c.color,bat:Math.round(oppStr(c).bat*100)/100,pitch:Math.round(oppStr(c).pitch*100)/100,def:Math.round(oppStr(c).def*100)/100,W:0,L:0,starter:j%5}});
 const d0=new Date(DATA.date0+"T00:00:00");const sched=[];let g=1,day=0;while(g<=30){const d=new Date(d0);d.setDate(d0.getDate()+day);if(d.getDay()!==1){const oi=Math.floor((g-1)/3)%opps.length;sched.push({g,date:isoLocal(d),opp:opps[oi].id,home:Math.floor((g-1)/3)%2===0,result:null});g++}day++}
...
function today(){return isoLocal(dateOf(S.day))}
...
function moraleF(p){return 0.94+0.12*moraleOverall(p)}
function batStrength(){const L=S.lineup.map(P);let s=0;for(let i=0;i<9;i++){const p=L[i];s+=(p&&!p.injury&&p.type=="B")?(0.4*p.attrs.contact+0.3*p.attrs.power+0.3*p.attrs.eye)*condF(p)*moraleF(p):0.33}return s/9}   // an empty or injured slot bats like a replacement player
...
function defStrength(){const L=S.lineup.map(P).filter(p=>p&&!p.injury);if(!L.length)return 0.4;const w=positionWarnings();const oop=L.filter(p=>p.pos0&&p.pos!==p.pos0&&p.pos!=="DH").length;return Math.max(0.2,L.reduce((s,p)=>s+0.7*p.attrs.defense+0.3*p.attrs.arm,0)/L.length-0.05*(w.miss.length+w.dup.length)-0.03*oop)}
function pitchStrength(sp){const pen=1-0.6*Math.max(0,sp.fatigue-0.3);const st=(0.5*sp.attrs.stuff+0.3*sp.attrs.control+0.2*sp.attrs.movement)*condF(sp)*pen*moraleF(sp);
...
function playGame(g){const opp=S.opps.find(o=>o.id===g.opp);const sp=nextStarter();const ps=pitchStrength(sp);const B=batStrength(),D=defStrength();
 ...
 sp.stats.G++;sp.stats.IP=Math.round((sp.stats.IP+ip)*10)/10;sp.stats.ER+=er;sp.stats.KP+=Math.round(ip*(0.6+0.6*sp.attrs.stuff));if(ru>rt)sp.stats.W++;else sp.stats.L++;sp.fatigue=Math.min(1,sp.fatigue+0.28+0.015*ip);sp.condition=Math.max(30,sp.condition-4);S.weekGames[sp.id]++;
 const rps=S.players.filter(p=>p.type=="P"&&p.role=="RP"&&p.active&&!p.injury);for(let k=0;k<Math.min(3,rps.length);k++){const r=rps[Math.floor(rng()*rps.length)];r.stats.G++;r.stats.IP=Math.round((r.stats.IP+1)*10)/10;r.stats.ER+=Math.round((rt-er)/Math.min(3,rps.length));r.stats.KP+=Math.round(0.6+0.6*r.attrs.stuff);r.fatigue=Math.min(1,r.fatigue+0.14);if(k===0&&ru>rt&&ru-rt<=3)r.stats.SV++}
 ...
 const others=S.opps.filter(o=>o.id!==opp.id).slice();for(let i=others.length-1;i>0;i--){const j=Math.floor(rng()*(i+1));[others[i],others[j]]=[others[j],others[i]]}
 for(let i=0;i+1<others.length;i+=2){const a=others[i],b=others[i+1];const pw=0.5+0.6*((a.bat+a.pitch)-(b.bat+b.pitch));if(rng()<pw){a.W++;b.L++}else{a.L++;b.W++}}}
...
function trainDay(){const isGame=!!gameToday()||!!S._gameDay;const gains={};const staff=S.staff;
 S.players.forEach(p=>{if(p.injury){p.injury--;p.fatigue=Math.max(0,p.fatigue-0.05);if(p.injury===0)addLog(p.name+" 부상 회복, 복귀");return}
  ...
  const risk=(0.0015+0.02*Math.max(0,p.fatigue-0.6)+prog.risk*0.5*(0.5+p.fatigue))*inten;if(rng()<risk){p.injury=3+Math.floor(rng()*(8+30*p.fatigue));addLog(p.name+" 훈련 중 부상 · "+p.injury+"일 결장");}
 });
...
function recoverDay(){const gd=!!gameToday()||!!S._gameDay;S.players.forEach(p=>{p.fatigue=Math.max(0,p.fatigue-(gd?0.06:0.10));p.condition=Math.min(99,p.condition+(gd?0:2));p.condition=Math.min(99,Math.max(30,p.condition+(rng()-0.5)*2))})}
...
/* a game day cannot be skipped: the manager plays it in the game screen (or ends it there with 결과 바로보기). force=true quick-sims it (smoke tests only) */
function advanceDay(force){if(S.day>=DATA.days){addLog("시즌 종료. 최종 "+S.W+"승 "+S.L+"패");render();return}
 const g0=gameToday();if(g0&&!(S.played||[]).includes(g0.g)&&!g0.result&&!force){addLog("오늘은 경기가 있습니다 · 경기 화면에서 치르세요 (넘길 수 없음)");render();return}
 if(S.policy!=="manual"&&S.day%7===0)applyPolicy();
 const g=gameToday();const M=window.ClubMarket;S._gameDay=!!g||!!(S.played||[]).includes((S.sched.find(x=>x.date===today())||{}).g);if(g&&!(S.played||[]).includes(g.g)){playGame(g);if(g.home&&M)M.gate(g)}trainDay();recoverDay();S._gameDay=false;if(S.day%7===6){moraleWeek();if(M)M.weekly()}S.day++;
...
function rankNow(){const rows=[{name:(S.club&&S.club.name)||"우리 구단",W:S.W,L:S.L,D:S.D||0,me:true}].concat(S.opps.map(o=>({name:o.name,W:o.W,L:o.L,D:o.D||0})));rows.sort((a,b)=>(b.W/(b.W+b.L||1))-(a.W/(a.W+a.L||1)));return rows}
function render(){if(window.APP.theme&&S.club)window.APP.theme(S.club.color,S.club.color2);S.lineup=S.lineup.map((id,i)=>{const q=P(id);return q&&q.type=="B"&&S.lineup.indexOf(id)===i?id:null});const d=dateOf(S.day);$("tDate").textContent=fmt(d);$("tRec").textContent=S.W+"-"+S.L+(S.D?"-"+S.D:"");const rows=rankNow();$("tRank").textContent=(rows.findIndex(r=>r.me)+1)+"위 / "+rows.length;$("tBudget").textContent=S.budget.toFixed(1);$("tPay").textContent=top40().toFixed(1)+"/137.4";
 $("tMorale").textContent=(S.players.length?Math.round(100*S.players.reduce((s,p)=>s+moraleOverall(p),0)/S.players.length):0)+"%";
 renderSchedule();renderTraining();renderRoster();renderStats();if(window.ClubMarket)window.ClubMarket.render();const gt=gameToday();const pend=gt&&!gt.result&&!(S.played||[]).includes(gt.g);$("nextDay").textContent=pend?"오늘 경기 시작":"다음 날";$("nextDay").classList.toggle("go",true);$("nextDay").disabled=S.day>=DATA.days}
function renderSchedule(){const t=today();if(S._schedDay!==S.day){S._schedDay=S.day;const ti=Math.max(0,S.sched.findIndex(g=>g.date>=t));PG.sched=Math.floor(ti/8)}const rows=S.sched.map(g=>{const o=S.opps.find(x=>x.id===g.opp);const r=g.result;return "<tr class='"+(g.date===t?"today":"")+"'><td>"+g.g+"</td><td>"+fmt(new Date(g.date+"T00:00:00"))+"</td><td>"+o.name+"</td><td>"+(g.home?"홈":"원정")+"</td><td>"+(r?"<span class='"+(r.us>r.them?"win":r.us<r.them?"loss":"draw")+"'>"+(r.us>r.them?"승":r.us<r.them?"패":"무")+" "+r.us+":"+r.them+"</span>":"-")+"</td><td>"+(r?r.sp+" "+r.ip+"이닝 "+r.er+"자책":"")+"</td></tr>"});
...
function renderRoster(){const bats=S.players.filter(p=>p.type=="B"&&p.active&&!p.injury);
 ...
 const byB=(a,b)=>(a.type=="B"?0:1)-(b.type=="B"?0:1);const act=S.players.filter(p=>p.active).sort(byB),res=S.players.filter(p=>!p.active).sort(byB);
...
function renderStats(){const rows=rankNow();$("standings").innerHTML="<table><tr><th>#</th><th>구단</th><th class='num'>승</th><th class='num'>패</th><th class='num'>무</th><th class='num'>승률</th></tr>"+rows.map((r,i)=>"<tr"+(r.me?" class='today'":"")+"><td>"+(i+1)+"</td><td>"+r.name+"</td><td class='num'>"+r.W+"</td><td class='num'>"+r.L+"</td><td class='num'>"+(r.D||0)+"</td><td class='num'>"+((r.W/((r.W+r.L)||1))).toFixed(3)+"</td></tr>").join("")+"</table>";
...
$("nextDay").onclick=()=>{const g=gameToday();if(g&&!g.result&&!(S.played||[]).includes(g.g)){if(window.APP.show)window.APP.show("game")}else advanceDay()};
$("reset").onclick=()=>{if(confirm("저장된 시즌을 지우고 새로 시작할까요?")){const c=S&&S.club;fresh(c&&c.code?{name:c.name,color:c.color,color2:c.color2,city:c.city,code:c.code}:undefined);render()}};
...
function recordExternalGame(res){const g=gameToday();if(!g||g.result)return false;const opp=S.opps.find(o=>o.id===g.opp);g.result={us:res.us,them:res.them,sp:res.sp||"-",ip:res.ip||0,er:res.er||0,real:true};if(res.us>res.them){S.W++;opp.L++}else if(res.us<res.them){S.L++;opp.W++}else{S.D=(S.D||0)+1;opp.D=(opp.D||0)+1}if(g.home&&window.ClubMarket)window.ClubMarket.gate(g);S.runs+=res.us;S.ra+=res.them;S.played=S.played||[];S.played.push(g.g);
 ...
 if(res.sp){const p=S.players.find(x=>x.name===res.sp);if(p){p.stats.G++;p.stats.IP=Math.round((p.stats.IP+(res.ip||0))*10)/10;p.stats.ER+=res.er||0;if(res.us>res.them)p.stats.W++;else if(res.us<res.them)p.stats.L++;p.fatigue=Math.min(1,p.fatigue+0.28+0.015*(res.ip||0))}}S.rotIdx=(S.rotIdx+1)%Math.max(1,S.rotation.length);
 addLog((res.us>res.them?"승 ":res.us<res.them?"패 ":"무 ")+res.us+":"+res.them+" vs "+opp.name+" · 경기 화면");save();render();return true}
window.ClubUI={state:()=>S,render,advanceDay,simDay:()=>advanceDay(true),fresh,setState:s=>{S=s;save();render()},recordExternalGame,gameToday:()=>{const g=gameToday();return g?{g:g.g,opp:S.opps.find(o=>o.id===g.opp).name,home:g.home,starter:S.opps.find(o=>o.id===g.opp).starter}:null},lineupForGame:()=>{if(!S)return null;let names=null,oppNames=null,oppClub=null;if(S.kbo&&window.APP.kbo){names={};S.players.filter(p=>p.engine_id!==undefined&&p.active&&!p.injury).forEach(p=>{names[(p.type=="B"?"b":"p")+p.engine_id]={name:p.name,pos:p.pos,num:p.num}});const g=gameToday()||S.sched.find(x=>!x.result);const oc=g?S.opps.find(o=>o.id===g.opp):null;const kc=oc?window.APP.kbo.clubs.find(c=>c.code===oc.code):null;if(kc){oppClub={name:kc.name,short:kc.short,color:kc.color};const R=window.APP.roster;const ob=kc.players.filter(p=>p.group!=="P"&&p.active);const used=new Set();oppNames={};R.opp.batters.forEach(ab=>{let c=ob.find(p=>!used.has(p.name)&&(p.bats==="L")===(ab.hand==="L"))||ob.find(p=>!used.has(p.name));if(c){used.add(c.name);oppNames[ab.id]={name:c.name,pos:c.pos,num:c.num}}});const op=kc.players.filter(p=>p.group==="P"&&p.active);R.opp.pitchers.forEach((ap,i)=>{const c=op.filter(p=>(p.throws==="L")===(ap.hand==="L"))[i%Math.max(1,op.length)]||op[i%Math.max(1,op.length)];if(c)oppNames["p"+ap.id]={name:c.name,num:c.num}})}}const gh=gameToday();const home=!!(gh&&gh.home);const pitcherState={};S.players.filter(p=>p.type=="P"&&p.engine_id!==undefined).forEach(p=>{pitcherState[p.engine_id]={fatigue:p.fatigue,condition:p.condition,injury:p.injury}});const ids=S.lineup.map(id=>{const p=S.players.find(x=>x.id===id);return p&&p.engine_id!==undefined?p.engine_id:null});const sp=S.rotation.map(id=>S.players.find(x=>x.id===id)).filter(p=>p&&p.engine_id!==undefined&&!p.injury)[S.rotIdx%Math.max(1,S.rotation.length)];const reliefNames=S.players.filter(p=>p.type=="P"&&p.active&&!p.injury&&p.role!="SP").sort((a,b)=>ovr(b)-ovr(a)).slice(0,4).map(p=>({name:p.name,num:p.num}));let oppReliefNames=[];if(S.kbo&&window.APP.kbo){const g3=gameToday()||S.sched.find(x=>!x.result);const oc3=g3?S.opps.find(o=>o.id===g3.opp):null;const kc3=oc3?window.APP.kbo.clubs.find(c=>c.code===oc3.code):null;if(kc3)oppReliefNames=kc3.players.filter(p=>p.group==="P"&&p.active).slice(5,9).map(p=>({name:p.name,num:p.num}))}
return {home,reliefNames,oppReliefNames,order:ids.every(x=>x!==null)?ids:null,pitcher:sp?sp.engine_id:null,pitcherState,names,oppNames,oppClub,clubName:S.club&&S.club.name}}};
